...
## Phase 16 — Proof of payment, CSV import, due-date visibility (20 Sep 2026) — branch `phase-16-proofs-import`

**Shipped**
- Proofs (#13): migration 000018 `payment_proofs` + bucket `proofs`; renter `POST /me/proofs/upload` (one-shot Redis ticket) → presigned PUT → `POST /me/proofs` (size/type re-checked against MinIO), `GET /me/proofs`, withdraw while submitted; landlord `GET /proofs` (oldest first), `/proofs/summary`, `/proofs/{id}` (300 s view link, audited), `accept` (reuses `allocatePayment`, 409s propagate byte-identically, thank-you SMS), `reject {reason}` → new kind `proof_rejected` (SW/EN platform template, org-overridable). Rate limits 30 presigns/h, 10 submissions/24 h (Redis + Postgres). Renter app: proof sheet (camera capture, ≤5 MiB, instructions repeated inside), "Awaiting confirmation" chip, rejection reason + Send again, proof history + withdraw. Landlord app: Proofs tab (first while anything waits, count badge in nav + bottom bar), review sheet with viewer + claimed-vs-expected, Accept through the parameterised Record-payment sheet, Reject form; proofs sections on renter and contract pages.
- Imports (#14): migration 000019 `import_batches`/`import_rows` + `payments.import_batch_id`; `GET /imports/templates/{kind}[.csv]`, `POST /imports/preview` (multipart, ≤2 MiB, ≤5 000 rows, BOM + `,`/`;` sniffed, per-cell errors + resolved hints), `POST /imports/{id}/commit` (one transaction, re-resolves, `skip_errors`), `undo` within 24 h, history. Kinds `units`, `renters` (→ `pending_signature`, never activates, `contract_ready` only on commit), `payments` (allocated in `paid_at` order, finished contracts allowed, "imported" chip). Settings → Import data page: kind cards, column reference + template download, drop zone, sticky preview table with Errors-only filter, commit / skip-and-commit, undo, history with read-only reopen.
- Visibility (#15): `days_until_due` + `next_due.proof` on `/me/schedules`; `GET /reports/upcoming?days=7|14|30`; `upcoming_7d` on `/reports/summary`; next-due fields on renters list/detail and units board; dashboard cards `upcoming` + `proofs`. Renter home hero (big date, amount, countdown chip, How to pay + Send proof); landlord Due-soon tab now reads the upcoming report with a 7/14/30 picker (fixes the old pending-only query), Next due column (sortable) on Renters, stat tiles on renter detail, due chips on unit cards.
- Instructions (#16): `mobile_money` beside `bank_account` (omit keeps, null clears) + `payment_instructions_set`; `{{pay_link}}` variable used by the reminder defaults (SW/EN reworded to stay one segment); pinned collapsible "How to pay" card at the top of the renter Payments tab (also reachable from home, contract page, proof sheet; fallback copy when unset); landlord bank-account page gains mobile money + live renter preview; dismissable nudge on dashboard/setup until set.
- Review pass (Opus): fixed a preview that leaked platform-wide names by phone, an undo that could delete a user another org had linked, undo gating by batch kind, link-request withdrawal only when the contract was withdrawn, Postgres backstop on the preview limit; added parity/double-accept/reverse-after-accept/single-use-ticket tests.
- Infra: `minio/minio:latest` no longer published → compose pinned to the cached release tags. Send-hour scheduler test made clock-relative.

**Verified**: `make build/test/lint/test-isolation` green; curl: proof upload → submit → badge → accept → schedule paid; units CSV preview (1 error) → 409 → skip-commit → undo; browser (375 px renter): hero, pinned card, proof sheet, rejection reason + Send again; browser (landlord): dashboard cards with real figures, Due soon 7/14/30, Proofs tab + badge, review sheet with image, Reject → REJECTED stamp, `proof_rejected` SMS body correct in the log; Import data page, Renters Next-due column.
